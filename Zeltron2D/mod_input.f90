...
! Spatial dimension
INTEGER, PARAMETER, PUBLIC :: NDIM=2

! If NDIM = 2, define some parameters in the z direction
...
INTEGER*8, PARAMETER, PUBLIC :: PPC=2

! Number of process (domain decomposition in the X- and Y-directions)
...
INTEGER, PARAMETER, PUBLIC :: FDUMP=500

! Number of data dumps
INTEGER, PARAMETER, PUBLIC :: NDUMP=3
...
! Switch ON/OFF spatial filtering
LOGICAL, PARAMETER, PUBLIC :: FILTER=.FALSE.

! Smoothing parameter
DOUBLE PRECISION, PARAMETER, PUBLIC :: alpha=1.0/16.0
...
! Total number of particles per species NP=NCX*NCY*PPC
INTEGER*8, PARAMETER, PUBLIC :: NP=NCX*NCY*PPC
...
DOUBLE PRECISION, PARAMETER, PUBLIC :: dt=0.99*dx*dy/sqrt(dx*dx+dy*dy)/c
...
! The time step from which particle-tracking should start
...
! Particle's 4-velocity grid for the spectrum
...
INTEGER, PARAMETER, PUBLIC :: NFIELDX=100,NFIELDY=100

! Spatial grid for dumping particle-related fields (densities, etc.)
INTEGER, PARAMETER, PUBLIC :: NDX=100,NDY=100
...
DOUBLE PRECISION, PARAMETER, PUBLIC :: pmin=-89.99,pmax=89.99

! Longitude boundaries for the angular distribution (lambda)
DOUBLE PRECISION, PARAMETER, PUBLIC :: lmin=-179.99,lmax=179.99
